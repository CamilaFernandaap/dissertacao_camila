...
Esta seção apresenta as normais climatológicas para os estados de São Paulo e Rio Grande do Norte calculadas para o período completo de análise (1990-2024), com base na expressão nos calculos empregados na Subseção \ref{sub:normais_climatologicas}.
...
A região metropolitada de São Paulo não se destaca como áreas de máximos, uma vez que a temperatura média é dominada por fatores de escala regional. Porém, especialmente nas estações DJF e MAM, o que se percebe é que a área correspondente à capital apresenta temperaturas ligeiramente superiores às regiões adjacentes de altitude semelhante, comporante de forma compatível com o efeito de ilha de calor urbana \citep{Campelo}. Sendo assim, ressalta-se que esse efeito atua em escala local e tende a ser modulado pela influência do relevo predominante.
...
Esta intensificação da precipitação costeira tem origem bem definida: o levantamento orográfico forçado das massas de ar úmidas de origem oceânica. À medida que essas massas ascendem as encostas da Serra do Mar, experimentam resfriamento adiabático que induz condensação e precipitação intensificada \citep{eagleson1978}, particularmente nas vertentes voltadas para o oceano. A região metropolitana de São Paulo, situada em posição intermediária, apresenta valores típicos de $500$ a $600$ mm sazonais.
...
A radiação solar incidente em superfície (Figura \ref{fig:climatologia_rad_sp}) apresenta padrões espaciais distintos daqueles observados para temperatura e precipitação. Durante o verão (DJF), quando o sol atinge sua posição mais alta no céu do hemisfério sul, os valores médios variam desde aproximadamente $180$ a $200$ W/m$^2$ no litoral sul até $240$ a $260$ W/m$^2$ no noroeste do estado.
...
\begin{figure}[htbp]
	...
	\caption[Normais climatológicas de temperatura do ar no Rio Grande do Norte]{Normais climatológicas de temperatura do ar no Rio Grande do Norte (1990-2024).}
	\label{fig:climatologia_temp_rn}
\end{figure}

As temperaturas médias anuais no Rio Grande do Norte variaram de $26$ °C no litoral até $28$ °C no interior semiárido, estabelecendo um gradiente consideravelmente menor que em São Paulo. Esta diferença reflete fundamentalmente a baixa latitude do estado, que minimiza as variações espaciais na radiação solar incidente. 

A amplitude térmica sazonal também foi menor, situando-se entre $3$ e $4$ °C entre verão e inverno. Esta característica é típica de climas tropicais, onde a variação da insolação ao longo do ano é menor que em latitudes mais altas.

O verão (DJF) apresentou as temperaturas mais elevadas, com valores médios frequentemente superiores a $27$ °C mesmo no litoral. O inverno (JJA), apesar de representar o período relativamente mais fresco, manteve temperaturas médias superiores a $24$ °C em praticamente todo o estado. Esta elevação consistente das temperaturas ao longo do ano distingue fundamentalmente o clima potiguar do paulista.
\newpage
\subsubsection*{Precipitação}
...
\begin{figure}[htbp]
	...
	\caption[Normais climatológicas de precipitação no Rio Grande do Norte]{Normais climatológicas de precipitação no Rio Grande do Norte (1990-2024).}
	\label{fig:climatologia_prec_rn}
\end{figure}

O contraste mais marcante nas normais climatológicas do Rio Grande do Norte manifestou-se na precipitação. A precipitação anual variou de $1.200$ a $1.500$ mm no litoral oriental até apenas $400$ a $700$ mm no interior semiárido, estabelecendo um gradiente horizontal extremamente pronunciado em distâncias relativamente curtas.

A concentração sazonal da precipitação no interior é um dos aspectos mais definidores do clima semiárido. Cerca de $70$ a $80\%$ do total anual concentrou-se em apenas três a quatro meses, tipicamente de fevereiro a maio, quando a Zona de Convergência Intertropical (ZCIT) atinge sua posição mais meridional \citep{kousky1980diurnal,marengo2017}. Esta concentração extrema cria um déficit hídrico severo durante oito a nove meses do ano, período em que a precipitação é praticamente ausente.
...
\subsubsection*{Radiação solar incidente em superfície}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{fig/Climatologia_Radiacao_RN_mapas.png}
	\caption[Normais climatológicas de radiação solar no Rio Grande do Norte]{Normais climatológicas de radiação solar no Rio Grande do Norte (1990-2024).}
	\label{fig:climatologia_rad_rn}
\end{figure}

A radiação solar no Rio Grande do Norte apresentou valores consistentemente elevados em todo o estado, variando entre $250$ e $280$ W/m$^2$ em média anual. Estes valores são substancialmente superiores aos observados em São Paulo, refletindo a posição tropical do estado e a alta transparência atmosférica característica do semiárido, particularmente durante a estação seca.

A variação espacial é modesta quando comparada com São Paulo. O interior semiárido apresentou valores ligeiramente superiores ao litoral, diferença que se deve principalmente à menor nebulosidade média no interior. Durante a estação seca, quando a nebulosidade é mínima, o interior pode receber valores de radiação que excedem $280$ W/m$^2$, estabelecendo um dos locais com maior incidência de radiação solar do Brasil.
...
Adicionalmente às médias climatológicas, foram calculadas medidas de variabilidade incluindo desvio padrão ($\sigma$), valores máximos e mínimos observados ao longo do período para as três variáves climáticas, e percentis (P$_{25}$ e P$_{75}$) para caracterizar a distribuição estatística. A variabilidade sazonal foi visualizada com auxílio de gráficos boxplots, permitindo a identificação de tendência central, amplitude de variação, identificação de valores extremos e diferenças na dispersão entre as estações do ano.
...
\begin{figure}[htbp]
	...
	\caption[Variabilidade sazonal da precipitação no estado de São Paulo]{Variabilidade sazonal da precipitação no estado de São Paulo (1990-2024).}
	\label{fig:boxplot_prec_sp}
\end{figure}

A precipitação (Figura \ref{fig:boxplot_prec_sp}) contrasta com a temperatura em termos de variabilidade. As distribuições são fortemente assimétricas em todas as estações, com numerosos outliers superiores que se estendem a valores extremamente elevados. Essa assimetria reflete a heterogeneidade espacial do estado de São Paulo: regiões costeiras e serranas, que sofrem influência da orografia e do sistemas atlânticos, apresentam acumulados sazonais superiores ao interior e oeste do estado \citep{marengo2009extreme}, de modo que o boxplot agrega regimes de precipitação distintos em uma única distribuição.

A amplitude interquartil também varia substancialmente entre estações, desde aproximadamente $100$ mm em SON até $250$ a $300$ mm em DJF, evidenciando não apenas diferentes magnitudes de precipitação, mas também diferentes regimes de variabilidade espacial e temporal entre as estações do ano.
...
\begin{figure}[htbp]
	...
	\caption[Variabilidade sazonal da precipitação no Rio Grande do Norte]{Variabilidade sazonal da precipitação no Rio Grande do Norte (1990-2024).}
...
Na estação seca (JJA-SON) o que se observa são medianas extremamente baixas (~$10$ a $20$ mm por trimestre), apesar disso, para JJA o que se observou foi a presenda de valores acima de 300 mm mesmo em uma estação caracterizada por ser mais seca, enquanto qe para SON os valores não ultrapassam 400 mm, sendo este intervalo mas retrito à precipitação quando comparado ao anterior.
...
\begin{table}[htbp]
	...
	\begin{tabular}{llcc}
		\hline
		\textbf{Região} & \textbf{Município} & \textbf{Latitude} & \textbf{Longitude} \\
		\hline
        Noroeste & Ouroeste          & -19{,}8634 & -50{,}4096 \\
        Norte    & Franca            & -20{,}5102 & -47{,}4182 \\
        Nordeste & Caconde           & -21{,}4803 & -46{,}5289 \\
        Oeste    & Teodoro Sampaio   & -22{,}2888 & -52{,}5925 \\
        Centro   & Botucatu          & -22{,}9356 & -48{,}4693 \\
        Leste    & Cruzeiro          & -22{,}5314 & -44{,}9928 \\
        Sudoeste & Rosana            & -22{,}6122 & -53{,}0776 \\
        Sul      & São Miguel Arcanjo& -23{,}9866 & -47{,}9842 \\
        Sudeste  & Peruíbe           & -24{,}2292 & -46{,}9331 \\
        Capital  & São Paulo         & -23{,}5015 & -46{,}6098 \\
		\hline
	\end{tabular}
\end{table}
...
A primavera (SON) destaca-se como a estação com aquecimento mais intenso e estatisticamente significativo em praticamente todas as localidades. As regiões do interior norte e nordeste apresentam as tendências mais elevadas, com Franca registrando 0${,}$073 °C/ano (p$<$0${,}$01), Caconde 0${,}$070 °C/ano (p$<$0${,}$01) e Teodoro Sampaio 0${,}$067 °C/ano (p$<$0${,}$05). O município de Ouroeste, no noroeste paulista, apresentou tendência de 0${,}$064 °C/ano (p$<$0${,}$01), valor comparável ao observado em Botucatu (0${,}$064 °C/ano, p$<$0${,}$05) na região central. Este padrão sugere que a primavera, período de transição entre a estação seca e o início da estação chuvosa, é particularmente sensível às mudanças no balanço de energia superficial, possivelmente amplificadas pela redução da umidade do solo ao final do inverno e pelo aumento progressivo da radiação solar \citep{alkama2016biophysical, shen2015dryland}.

Durante o inverno (JJA), observa-se aquecimento significativo nas regiões do interior oeste e noroeste, com destaque para Teodoro Sampaio (0${,}$035 °C/ano, p$<$0${,}$05) e Ouroeste (0${,}$033 °C/ano, p$<$0${,}$05), ambos localizados em áreas de menor influência oceânica onde a amplitude térmica sazonal é mais acentuada. Rosana, também no oeste paulista, apresentou tendência idêntica à de Teodoro Sampaio (0${,}$035 °C/ano, p$<$0${,}$05). Nessas regiões, a menor influência oceânica favorece maior sensibilidade às mudanças no balanço de energia à superfície, resultando em tendências mais pronunciadas \citep{Nobre2011}. As demais localidades apresentaram tendências positivas durante o inverno, porém sem significância estatística consistente, com exceção de Botucatu que registrou aquecimento de 0${,}$026 °C/ano, embora sem significância ao nível de 5\%.

O verão (DJF) caracterizou-se por tendências predominantemente positivas, porém com menor número de valores estatisticamente significativos quando comparado às demais estações. Teodoro Sampaio e Rosana mantiveram-se entre as localidades com aquecimento mais pronunciado, registrando 0${,}$022 °C/ano (p$<$0${,}$05) e 0${,}$023 °C/ano (p$<$0${,}$05), respectivamente. Ouroeste apresentou tendência de 0${,}$019 °C/ano (p$<$0${,}$05). As regiões litorâneas e próximas à Serra do Mar, representadas por Peruíbe (0${,}$006 °C/ano), São Miguel Arcanjo (0${,}$002 °C/ano) e Cruzeiro (0${,}$009 °C/ano), exibiram aquecimento mais moderado sem significância estatística, reflexo da influência termorreguladora do oceano Atlântico e da maior frequência de nebulosidade associada a sistemas convectivos de verão.

No outono (MAM), as tendências foram em geral positivas mas apresentaram menor significância estatística na maioria das localidades. Botucatu apresentou uma tendência de 0${,}$030 °C/ano (p$<$0${,}$05), a única estatisticamente significativa nesta estação do ano. Teodoro Sampaio (0${,}$028 °C/ano) e Rosana (0${,}$030 °C/ano) apresentaram valores elevados porém sem significância. Este comportamento evidenciou um fenômeno importante: em determinadas localidades e estações do ano, a elevada variabilidade interanual da temperatura pode mascarar o sinal de tendência, reduzindo a significância estatística do teste de Mann-Kendall mesmo quando as inclinações são relativamente elevadas \citep{vonstorch1999,yue2002,wilks2011}.

A Região Metropolitana de São Paulo, representada pela capital, apresentou aquecimento moderado mas consistente ao longo de todas as estações, com tendências variando entre 0${,}$007 °C/ano no verão e 0${,}$047 °C/ano (p$<$0${,}$05) na primavera. Embora as taxas não tenham sido as mais elevadas do estado, o padrão persistente de aquecimento ao longo de todo o ano foi fortemente associado ao efeito de ilha de calor urbana, impermeabilização do solo e alterações nas propriedades radiativas da superfície \citep{Campelo}. Estudos prévios indicam que a urbanização pode amplificar o sinal térmico regional, especialmente durante a noite e nas estações mais quentes do ano.

\subsection{Radiação Solar}

Os resultados de radiação solar incidente apresentados nas Tabelas \ref{tab:tendencias_radiacao_sp_a} e \ref{tab:tendencias_radiacao_sp_b} indicaram tendência predominante de aumento ao longo do período analisado, com variações regionais e sazonais que refletiram mudanças nos padrões de nebulosidade, propriedades ópticas da atmosfera e circulação atmosférica regional \citep{wild2012, yamasoe2021}.
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		...
		\textbf{Município} & \textbf{DJF} & \textbf{JJA} & \textbf{MAM} & \textbf{SON} \\
		\hline
		Ouroeste & 0${,}$60*& 0${,}$34** & 0${,}$26*& 0${,}$37*\\
		Franca& 1${,}$12** & 0${,}$74** & 0${,}$37** & 0${,}$74** \\
		Caconde  & 0${,}$87** & 0${,}$50*& 0${,}$24*& 0${,}$62** \\
		Teodoro Sampaio & 0${,}$61** & 0${,}$46*& 0${,}$16& 0${,}$45*\\
		Botucatu   & 0${,}$57*& 0${,}$44*& 0${,}$20& 0${,}$43** \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		...
		\textbf{Município} & \textbf{DJF} & \textbf{JJA} & \textbf{MAM} & \textbf{SON} \\
		\hline
		Cruzeiro& 1${,}$12** & 0${,}$54** & 0${,}$09& 0${,}$51*\\
		Rosana   & 0${,}$47& 0${,}$26& 0${,}$06& 0${,}$42\\
		São Miguel Arcanjo & 0${,}$35& 0${,}$31& 0${,}$05& 0${,}$20\\
		Peruíbe   & 0${,}$88** & 0${,}$76** & 0${,}$24& 0${,}$43*\\
		São Paulo & 0${,}$88** & 0${,}$59*& 0${,}$13& 0${,}$42*\\
		\hline
	\end{tabular}
	\vspace{2mm}
...
As localidades do interior norte e nordeste apresentaram os aumentos mais pronunciados e estatisticamente significativos em praticamente todas as estações.

Durante o inverno (JJA), período caracterizado por menor nebulosidade e maior estabilidade atmosférica, observou-se aumento generalizado e estatisticamente significativo (menos São Miguel Arcanjo) de radiação solar em todas as regiões do interior. Além dos valores já mencionados para Franca e Cruzeiro, destacaram-se Caconde (0${,}$50 W/m$^2$/ano, p$<$0${,}$05), Teodoro Sampaio (0${,}$46 W/m$^2$/ano, p$<$0${,}$05), Botucatu (0${,}$44 W/m$^2$/ano, p$<$0${,}$05) e Ouroeste (0${,}$34 W/m$^2$/ano, p$<$0${,}$01).

O verão (DJF), apesar de ter-se caracterizado por maior nebulosidade associada a sistemas convectivos, também apresentou tendências positivas significativas em várias localidades. Além de Franca, Cruzeiro e Caconde já mencionados, destacaram-se a Região Metropolitana de São Paulo (0${,}$88 W/m$^2$/ano, p$<$0${,}$01), Peruíbe (0${,}$88 W/m$^2$/ano, p$<$0${,}$01), Teodoro Sampaio (0${,}$61 W/m$^2$/ano, p$<$0${,}$01), Ouroeste (0${,}$60 W/m$^2$/ano, p$<$0${,}$05) e Botucatu (0${,}$57 W/m$^2$/ano, p$<$0${,}$05). A presença de tendências significativas mesmo durante o período mais chuvoso do ano indicou que o sinal de aumento da radiação superou a variabilidade natural associada aos padrões de precipitação.

A zona litorânea, representada por Peruíbe, apresentou comportamento distinto com tendências positivas altamente significativas no verão (0${,}$877 W/m$^2$/ano, p$<$0${,}$01) e inverno (0${,}$77 W/m$^2$/ano, p$<$0${,}$01), mas sem significância estatística no outono e primavera. Este padrão refletiu a maior variabilidade interanual da radiação solar na costa, associada à maior frequência de nebulosidade relacionada à circulação marítima e à passagem de sistemas frontais. A Região Metropolitana de São Paulo apresentou comportamento intermediário, com tendências significativas no verão (0${,}$88 W/m$^2$/ano, p$<$0${,}$01), inverno (0${,}$59 W/m$^2$/ano, p$<$0${,}$05) e primavera (0${,}$42 W/m$^2$/ano, p$<$0${,}$05).

No outono (MAM), as tendências foram predominantemente positivas mas com menor número de valores estatisticamente significativos. Destacaram-se Franca (0${,}$372 W/m$^2$/ano, p$<$0${,}$01), Ouroeste (0${,}$26 W/m$^2$/ano, p$<$0${,}$05) e Caconde (0${,}$240 W/m$^2$/ano, p$<$0${,}$05). A primavera (SON) apresentou padrão similar ao inverno, com tendências positivas significativas em Franca (0${,}$74 W/m$^2$/ano, p$<$0${,}$01), Caconde (0${,}$62 W/m$^2$/ano, p$<$0${,}$01), Cruzeiro (0${,}$51 W/m$^2$/ano, p$<$0${,}$05), Teodoro Sampaio (0${,}$45 W/m$^2$/ano, p$<$0${,}$05), Peruíbe (0${,}$43 W/m$^2$/ano, p$<$0${,}$05), Botucatu (0${,}$43 W/m$^2$/ano, p$<$0${,}$01), São Paulo (0${,}$42 W/m$^2$/ano, p$<$0${,}$05) e Ouroeste (0${,}$37 W/m$^2$/ano, p$<$0${,}$05).

De modo geral, os resultados indicaram aumento sistemático da disponibilidade de energia radiativa na superfície do estado de São Paulo ao longo das últimas três décadas, com acumulados da ordem de 15 a 40 W/m$^2$ dependendo da estação e localidade.

\subsection{Precipitação}

A análise das tendências de precipitação acumulada apresentadas nas Tabelas \ref{tab:tendencias_prec_sp_a} e \ref{tab:tendencias_prec_sp_b} revelou padrão predominantemente negativo em praticamente todas as localidades e estações, embora a maioria das tendências não atingisse significância estatística devido à elevada variabilidade interanual característica do regime pluviométrico do estado de São Paulo. Este comportamento contrastou fortemente com as tendências espacialmente coerentes observadas para temperatura e radiação solar, refletindo a complexidade dos processos que governam a precipitação, fortemente influenciados pela alta sazonalidade \citep{alvares2013, Groisman2005}.
...
O verão (DJF), estação responsável por aproximadamente metade da precipitação anual no estado, apresentou tendências predominantemente negativas em oito das dez localidades analisadas. São Miguel Arcanjo, localizado na região sul próximo à Serra de Paranapiacaba, registrou a única tendência estatisticamente significativa nesta estação (-6${,}$11 mm/ano, p$<$0${,}$05), representando redução acumulada de aproximadamente 214 mm ao longo dos 35 anos do período analisado. Outras localidades apresentaram reduções expressivas porém sem significância estatística, como Caconde (-4${,}$74 mm/ano), São Paulo (-4${,}$11 mm/ano), Botucatu (-3${,}$94 mm/ano), Ouroeste (-3${,}$61 mm/ano) e Franca (-3${,}$36 mm/ano). As exceções foram Cruzeiro, que apresentou tendência positiva de 2${,}$14 mm/ano sem significância, e Rosana (0${,}$51 mm/ano) e Teodoro Sampaio (-0${,}$01 mm/ano), ambos no oeste paulista com valores próximos de zero.

Durante o inverno (JJA), período naturalmente mais seco quando a precipitação é escassa e dominada pela passagem episódica de sistemas frontais, as tendências foram predominantemente negativas mas com magnitudes modestas. A Região Metropolitana de São Paulo apresentou a única redução estatisticamente significativa nesta estação (-3${,}$63 mm/ano, p$<$0${,}$05), possivelmente associada a mudanças nos padrões de circulação atmosférica ou à intensificação do efeito de ilha de calor urbana que pode afetar a formação e trajetória de sistemas precipitantes. Botucatu (-2${,}$70 mm/ano), Ouroeste (-1${,}$78 mm/ano), São Miguel Arcanjo (-1${,}$71 mm/ano) e Cruzeiro (-1${,}$23 mm/ano) também apresentaram reduções sem significância estatística. Teodoro Sampaio foi a única localidade com tendência positiva no inverno (0${,}$14 mm/ano), embora de magnitude desprezível.

O outono (MAM), período de transição entre a estação chuvosa e seca, caracterizou-se por tendências negativas em praticamente todas as localidades. Cruzeiro apresentou a única tendência estatisticamente significativa (-1${,}$02 mm/ano, p$<$0${,}$05), embora de magnitude relativamente modesta. Rosana (-1${,}$29 mm/ano), São Paulo (-1${,}$13 mm/ano), São Miguel Arcanjo (-1${,}$12 mm/ano), Franca (-0${,}$83 mm/ano) e Botucatu (-0${,}$77 mm/ano) também apresentaram reduções sem significância estatística. A menor contribuição absoluta da precipitação nesta estação, aliada à elevada variabilidade interanual, dificulta a detecção de sinais significativos de mudança de longo prazo.

A primavera (SON) apresentou o padrão mais heterogêneo entre todas as estações. Algumas localidades exibiram tendências ligeiramente positivas, como Caconde (0${,}$92 mm/ano), Franca (0${,}$45 mm/ano) e Ouroeste (0${,}$23 mm/ano), todas no interior norte e nordeste, enquanto outras apresentaram reduções como São Paulo (-2${,}$78 mm/ano), Rosana (-2${,}$68 mm/ano), Teodoro Sampaio (-1${,}$83 mm/ano), São Miguel Arcanjo (-1${,}$28 mm/ano) e Peruíbe (-1${,}$17 mm/ano). Nenhuma das tendências atingiu significância estatística nesta estação.

\section{Análise climática no estado de São Paulo}

\subsection{Tendências espaciais da temperatura média}

A análise espacial das tendências de temperatura do ar no estado de São Paulo para o período de 1990 a 2020 (Figura \ref{fig:tend_temp_sp}) evidenciou um sinal considerável, espacialmente coerente e estatisticamente significativo de aquecimento em praticamente todo o estado, como é possível observar com as hachuras em destaque nos quadros, independentemente da estação do ano considerada, com maior expressão no intervalo entre os meses de setembro, outubro e novembro (SON). As tendências foram estimadas a partir do método de Sen, com avaliação de significância estatística pelo teste de Mann-Kendall, aplicados às séries sazonais médias, considerando exclusivamente os pontos localizados dentro dos limites territoriais do estado.
...
O verão austral (DJF) apresentou aquecimento praticamente generalizado em todo o estado, com magnitudes típicas entre próximo do valor médio (sem mudança de sinal na tendência) e $\approx0{,}3$ °C/década \citep{abreu2019}. As maiores taxas concentraram-se nas regiões do interior oeste e noroeste, bem como em áreas densamente urbanizadas, incluindo a Região Metropolitana de São Paulo. Esse padrão sugeriu a atuação simultânea de forçantes climáticas de escala regional e global, associadas ao aumento da concentração de gases de efeito estufa \citep{ipcc2021}, e de fatores locais, como mudanças no uso e cobertura da terra \citep{alkama2016biophysical} e intensificação do efeito de ilha de calor urbana \citep{Almeida, oke1982energetic, manoli2019magnitude}. Em contraste, as regiões litorâneas apresentaram aquecimento ligeiramente mais moderado.

A transição para o outono (MAM) manteve o sinal de aquecimento consistente, embora com maior heterogeneidade espacial. As tendências situaram-se majoritariamente entre $0{,}1$ e $0{,}4$ °C por década, com áreas do interior e da porção norte do estado apresentando valores mais elevados.

Apesar de caracterizar-se por temperaturas médias mais baixas e menor atividade convectiva, o inverno austral (JJA) apresentou tendências de aquecimento comparáveis às observadas no verão em grande parte do território, com destaque em uma porção central distribuída entre as regiões de Piracicaba, Campinas e Sorocaba. As taxas observadas variaram tipicamente entre $0{,}25$ e $0{,}4$ °C por década, com ampla significância estatística. A persistência do aquecimento durante o inverno indicou que os processos responsáveis pela elevação da temperatura do ar atuaram de forma contínua ao longo do ano, não estando restritos a condições atmosféricas específicas \citep{abreu2019}.

Entre todas as estações, a primavera (SON) destacou-se pelo aquecimento mais intenso e espacialmente homogêneo em todo o estado. As tendências frequentemente excederam $0{,}4$ °C por década, especialmente nas regiões oeste, centro-oeste e norte. Esse padrão foi consistente com o papel da primavera como período de forte reorganização do sistema climático regional, marcado pelo aumento progressivo da radiação solar, elevação da temperatura do ar e retorno gradual da atividade convectiva após o inverno seco.

De forma geral, os resultados indicaram que o estado de São Paulo experimentou aquecimento acumulado da ordem de $0{,}6$ a $2{,}5$ °C ao longo das últimas três décadas e meia, dependendo da estação e da região considerada \citep{abreu2019}.

\subsection{Tendências espaciais da precipitação}
As tendências espaciais da precipitação no estado de São Paulo para o período de 1990 a 2020 (Figura \ref{fig:tend_prec_sp}) contrastaram fortemente com o comportamento observado para a temperatura do ar, apresentando padrão altamente heterogêneo e fragmentado, com ampla predominância de áreas sem significância estatística, conforme demonstrado para a análise pontual. As tendências são apresentadas em unidades decadais (mm por década).
...
O verão (DJF), estação responsável por aproximadamente metade da precipitação anual no estado \citep{alvares2013}, caracterizou-se pela ausência de um padrão regional consistente. Algumas áreas do interior apresentaram tendências positivas localizadas, enquanto outras indicaram reduções fracas, mas grande parte do território não exibiu tendências estatisticamente significativas. As magnitudes das tendências detectadas raramente excederam $\pm$40~mm por década, valores que, embora modestos em termos absoltos, podem representar mudanças substanciais considerando a variabilidade natural do sistema \citep{Groisman2005}.

No outono (MAM) revelaram-se tendências predominantemente fracas ou estatisticamente não significativas em quase todo o estado. A menor contribuição absoluta da precipitação nesta estação, aliada à elevada variabilidade interanual, dificulta a detecção de sinais significativos de mudança de longo prazo.

O período mais seco do ano, correspondente ao inverno (JJA), quando a precipitação é escassa, apresentou o maior número de áreas sem significância estatística, mas ainda poucas regiões. As tendências estimadas apresentaram baixas magnitudes e distribuição espacial irregular, comportamento consistente com a baixa frequência de eventos precipitantes nesta estação e com a dominância de variabilidade em termos da distribuição orográfica do estado.

Sinais mais coerentes de possíveis mudanças emergiram na primavera (SON), com diversas regiões do interior exibindo tendências negativas fracas a moderadas, com magnitudes típicas entre $-20$ e $-40$~mm por década. Embora sem significância estatística de forma individual, essas tendências formaram padrão espacial sugestivo de redução da precipitação neste período de transição entre a estação seca e o início do período chuvoso \citep{coelho2016}.

\subsection{Tendências espaciais da radiação solar}
As tendências de radiação solar incidente na superfície no estado de São Paulo para o período de 1990 a 2020 (Figura \ref{fig:tend_rad_sp}) apresentaram padrão predominantemente positivo e espacialmente coerente, em contraste com a elevada heterogeneidade observada para a precipitação. As tendências são expressas em unidades decadais (W/m$^2$ por década), representando a mudança na radiação solar média ao longo de dez anos.
\begin{figure}[htbp]
	\centering
...
As estações de verão (DJF) e outono (MAM) exibiram tendências predominantemente positivas em grande parte do território paulista, com magnitudes típicas entre 5 e 12~W/m$^2$ por década. O outono apresentou valores particularmente elevados, com amplas áreas mostrando valores superiores a 10~W/m$^2$ por década com significância estatística \citep{yamasoe2017, wild2012}.

Tendências positivas caracterizaram o inverno (JJA), com magnitudes frequentemente superiores a 8~W/m$^2$ por década e apenas algumas regiões fragmentadas no nordeste estendendo-se ao norte do estado de São Paulo com significância estatística. Algumas regiões ao sul do estado apresentaram tendências positivas moderadas ou próximas de zero, refletindo certa concordância com o perfil observado para precipitação no mesmo período, onde pontos de grade do sul indicaram elevação sutil de precipitação ou valores indiferentes à média do intervalo analisado. Esta relação inversa entre radiação e precipitação é fisicamente consistente, uma vez que maior precipitação está associada a maior cobertura de nuvens e consequente redução da radiação solar incidente \citep{Trenberth2009}.

A primavera (SON) manteve tendências majoritariamente positivas, embora com magnitudes ligeiramente inferiores ao verão e outono em algumas áreas, apesar desta estação indicar um período que precede o aumento de nebulosidade significativo.

De modo geral, os resultados indicaram aumento sistemático da disponibilidade de radiação solar incidente na superfície do estado de São Paulo ao longo das últimas três décadas, com incrementos acumulados da ordem de 15 a 35~W/m$^2$ dependendo da estação e localidade. Este padrão de \textit{brightening} regional \citep{wild2012, yamasoe2021} atuou em concordância com o aquecimento observado da temperatura do ar observada na Figura \ref{fig:tend_temp_sp}.

\section{Análise climática em pontos de grade para o estado do Rio Grande do Norte}

Para refletir a diversidade climática do Rio Grande do Norte, caracterizada pela transição entre o litoral úmido e o semiárido do interior, foram escolhidas dez localidades distribuídas pelo estado (Tabela \ref{tab:coordenadas_rn}), cada uma representando domínio climático distinto ou posição geográfica relevante para compreensão dos padrões espaciais do estado. A seleção considerou representatividade climática regional, abrangendo desde o Oeste Potiguar semiárido até a Região Metropolitana litorânea, permitindo capturar a heterogeneidade espacial das mudanças climáticas observadas neste estado nordestino.
...
\begin{table}[htbp]
	...
	\begin{tabular}{llcc}
		...
		\textbf{Região} & \textbf{Município} & \textbf{Latitude} & \textbf{Longitude} \\
		\hline
           Noroeste & Mossoró & -4{,}9873 & -37{,}3122\\
           Norte & Macau  & -5{,}2298 & -36{,}5037\\
           Nordeste & Rio do Fogo & -5{,}3107 & -35{,}3718\\
           Oeste & Severiano Melo & -5{,}7957 & -38{,}0398\\
           Centro & Santana do Matos & -5{,}7957 & -36{,}5037\\
           Capital & Parnamirim & -5{,}8766 & -35{,}2101\\
           Sudoeste & Alexandria & -6{,}3617 & -37{,}8781\\
           Centro-Sul & Currais Novos & -6{,}2000 & -36{,}5037\\
           Sul & Monte das Gameleiras& -6{,}4425 & -35{,}7761\\
          Leste & Nísia Floresta & -6{,}0383 & -35{,}1293\\
		\hline
	\end{tabular}
\end{table}
...
A análise das tendências de temperatura do ar apresentadas nas Tabelas \ref{tab:tendencias_temp_rn_a} e \ref{tab:tendencias_temp_rn_b} revelou padrão de aquecimento mais moderado e espacialmente heterogêneo quando comparado ao observado em São Paulo, com significância estatística limitada a poucas localidades e estações específicas. Este comportamento refletiu tanto a alta variabilidade interanual característica do clima semiárido quanto a complexa interação entre processos atmosféricos de escala regional, como a migração sazonal da Zona de Convergência Intertropical (ZCIT), e forçantes climáticas de larga escala.
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		...
		\textbf{Município} & \textbf{DJF} & \textbf{JJA} & \textbf{MAM} & \textbf{SON} \\
		\hline
		Mossoró  & 0${,}$02*& 0${,}$01& 0${,}$01& 0${,}$02*\\
		Macau& 0${,}$01& -0${,}$00& 0${,}$00& 0${,}$00\\
		Rio do Fogo & 0${,}$01& 0${,}$01& 0${,}$01& 0${,}$01*\\
		Severiano Melo & 0${,}$01& 0${,}$00& 0${,}$01& 0${,}$01\\
		Santana do Matos & 0${,}$01& -0${,}$01& 0${,}$00& 0${,}$01\\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		...
		\textbf{Município} & \textbf{DJF} & \textbf{JJA} & \textbf{MAM} & \textbf{SON} \\
		\hline
		Parnamirim & 0${,}$01& 0${,}$01 & 0${,}$01& 0${,}$01\\
		Alexandria & 0${,}$01& -0${,}$01& 0${,}$00& 0${,}$01\\
		Currais Novos & 0${,}$01& -0${,}$00& -0${,}$00& 0${,}$01\\
		Monte das Gameleiras & 0${,}$01& 0${,}$01& 0${,}$00& 0${,}$01\\
		Nísia Floresta & 0${,}$01& 0${,}$01& 0${,}$01& 0${,}$01\\
		\hline
	\end{tabular}
	\vspace{2mm}
...
O verão austral (DJF), estação que coincide com o início do período chuvoso no semiárido quando a ZCIT migra para sua posição mais meridional, apresentou as tendências de aquecimento mais pronunciadas. Mossoró, sendo o único local com tendência estatisticamente significativa, registrou tendência de 0${,}$02 °C/ano (p$<$0${,}$05), representando aquecimento acumulado de aproximadamente 0${,}$63 °C ao longo dos 35 anos analisados. Outras localidades apresentaram tendências positivas porém sem significância estatística, como Severiano Melo (0${,}$014 °C/ano), Macau (0${,}$01 °C/ano), Rio do Fogo (0${,}$01 °C/ano) e Parnamirim (0${,}$01 °C/ano), esta última representando a Região Metropolitana de Natal. As localidades do Seridó e Alto Oeste apresentaram as tendências mais fracas no verão, com Alexandria, Currais Novos e Monte das Gameleiras registrando valores entre 0${,}$02 e 0${,}$01 °C/ano.

A primavera (SON) apresentou-se como a estação com apenas duas tendências estatisticamente significativas, embora com magnitudes modestas. Mossoró manteve tendência significativa de 0${,}$02 °C/ano (p$<$0${,}$05), enquanto Rio do Fogo, no Agreste Potiguar costeiro, registrou 0${,}$01 °C/ano (p$<$0${,}$05). Este padrão sazonal, com aquecimento preferencial durante a primavera, pode estar relacionado aos mecanismos específicos desta época de transição que incluem mudanças nos padrões de circulação atmosférica regional e modificações no balanço radiativo associadas às condições de seca extrema ao final do período seco. Outras localidades demonstraram tendências positivas sem significância, como Monte das Gameleiras (0${,}$01 °C/ano), Santana do Matos (0${,}$01 °C/ano), Parnamirim (0${,}$01 °C/ano), Macau (0${,}$01 °C/ano) e Currais Novos (0${,}$01 °C/ano).

Durante o inverno (JJA), período de mínima precipitação quando o semiárido experimenta condições de extrema secura, as tendências térmicas foram predominantemente fracas e sem significância estatística na maioria das localidades. Rio do Fogo apresentou a tendência mais elevada nesta estação (0${,}$013 °C/ano), seguido por Parnamirim e Nísia Floresta (ambos com 0${,}$012 °C/ano), porém sem significância ao nível de 5\%. Chamou atenção a presença de tendências negativas não significativas em algumas localidades do interior, como Santana do Matos (-0${,}$01 °C/ano), Alexandria (-0${,}$01 °C/ano) e Currais Novos (-0${,}$00 °C/ano), além de Macau (-0${,}$00 °C/ano) na costa.

O outono (MAM), período que marca a transição da estação chuvosa para a seca e coincide com o pico da precipitação no interior semiárido, apresentou as tendências térmicas mais fracas e menos consistentes entre todas as estações. A maioria das localidades registrou valores próximos de zero ou ligeiramente positivos sem qualquer significância estatística. Destacaram-se Parnamirim (0${,}$01 °C/ano), Nísia Floresta (0${,}$01 °C/ano) e Rio do Fogo (0${,}$01 °C/ano), todos na faixa litorânea, com tendências modestas mas positivas. Currais Novos apresentou tendência praticamente nula (-0${,}$00 °C/ano), enquanto Macau registrou valor exatamente zero.

É importante destacar que a Região Metropolitana de Natal, representada por Parnamirim, apresentou padrão de aquecimento relativamente consistente ao longo de três das quatro estações, com tendências positivas entre 0${,}$010 e 0${,}$012 °C/ano no verão, inverno e outono, embora nenhuma atingisse significância estatística. Este comportamento sugeriu possível contribuição do efeito de ilha de calor urbana associado ao crescimento e adensamento urbano nas últimas décadas, embora o sinal tenha sido menos pronunciado que o observado na Região Metropolitana de São Paulo. Nísia Floresta, também no Leste Potiguar litorâneo, apresentou padrão similar com tendências positivas consistentes mas sem significância estatística.

\subsection{Radiação}

Os resultados de radiação solar incidente apresentados nas Tabelas \ref{tab:tendencias_rad_rn_a} e \ref{tab:tendencias_rad_rn_b} revelaram padrão notavelmente complexo e espacialmente heterogêneo, com tendências tanto positivas quanto negativas distribuídas de forma aparentemente irregular pelo território do Rio Grande do Norte.
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		...
		\textbf{Município} & \textbf{DJF} & \textbf{JJA} & \textbf{MAM} & \textbf{SON} \\
		\hline
		Mossoró  & -0${,}$12& 0${,}$49*& 0${,}$29*& -0${,}$23**\\
		Macau& 0${,}$06& 0${,}$95**& 0${,}$37**& -0${,}$23**\\
		Rio do Fogo & -0${,}$20& 0${,}$05& -0${,}$19& -0${,}$30*\\
		Severiano Melo & 0${,}$27**& 0${,}$54**& 0${,}$35*& 0${,}$40**\\
		Santana do Matos & 0${,}$66**& 1${,}$16***& 0${,}$56**& 0${,}$52***\\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		...
		\textbf{Município} & \textbf{DJF} & \textbf{JJA} & \textbf{MAM} & \textbf{SON} \\
		\hline
		Parnamirim & -0${,}$18& 0${,}$07& -0${,}$18& -0${,}$22*\\
		Alexandria & 0${,}$13& 0${,}$29*& 0${,}$32*& 0${,}$37**\\
		Currais Novos & 0${,}$59**& 0${,}$89***& 0${,}$37*& 0${,}$48***\\
		Monte das Gameleiras & -0${,}$11& 0${,}$55**& 0${,}$16& -0${,}$31*\\
		Nísia Floresta & -0${,}$28& 0${,}$02& -0${,}$13& -0${,}$32*\\
		\hline
	\end{tabular}
	\vspace{2mm}
...
O inverno austral (JJA), período de mínima nebulosidade quando a atmosfera apresenta máxima transparência radiativa, coincidindo com a menor ocorrência de precipitação no interior semiárido, destacou-se como a estação com maior número de tendências positivas altamente significativas. Santana do Matos, localizado na região Central Potiguar, apresentou o aumento mais pronunciado do estado (1${,}$16 W/m$^2$/ano, p$<$0${,}$001), representando um acumulado de aproximadamente 40 W/m$^2$ ao longo dos 35 anos analisados. Macau, no Agreste Potiguar costeiro, registrou tendência de 0${,}$95 W/m$^2$/ano (p$<$0${,}$01), enquanto Currais Novos, no Seridó, apresentou 0${,}$89 W/m$^2$/ano (p$<$0${,}$001). Outras localidades do interior também exibiram aumentos significativos, como Monte das Gameleiras (0${,}$55 W/m$^2$/ano, p$<$0${,}$01), Severiano Melo (0${,}$54 W/m$^2$/ano, p$<$0${,}$01) e Mossoró (0${,}$49 W/m$^2$/ano, p$<$0${,}$05). Em contraste, as localidades litorâneas apresentaram tendências fracas ou próximas de zero, como Alexandria (0${,}$29 W/m$^2$/ano, p$<$0${,}$05), Parnamirim (0${,}$07 W/m$^2$/ano) e Rio do Fogo (0${,}$05 W/m$^2$/ano), ambas sem significância.

O outono (MAM) apresentou padrão ainda mais complexo. Santana do Matos manteve-se como a localidade com maior tendência positiva (0${,}$56 W/m$^2$/ano, p$<$0${,}$01), seguida por Macau (0${,}$37 W/m$^2$/ano, p$<$0${,}$01), Currais Novos (0${,}$37 W/m$^2$/ano, p$<$0${,}$05), Severiano Melo (0${,}$35 W/m$^2$/ano, p$<$0${,}$05), Alexandria (0${,}$32 W/m$^2$/ano, p$<$0${,}$05) e Mossoró (0${,}$29 W/m$^2$/ano, p$<$0${,}$05). Em contraste com as localidades litorâneas, que apresentaram tendências negativas sem significância, como Rio do Fogo (-0${,}$19 W/m$^2$/ano), Parnamirim (-0${,}$18 W/m$^2$/ano) e Nísia Floresta (-0${,}$13 W/m$^2$/ano).

Durante o verão (DJF), as tendências foram predominantemente positivas no interior mas apresentaram significância estatística apenas em duas localidades. Santana do Matos novamente destacou-se com a tendência mais elevada (0${,}$66 W/m$^2$/ano, p$<$0${,}$01), seguida por Currais Novos (0${,}$59 W/m$^2$/ano, p$<$0${,}$01) e Severiano Melo (0${,}$27 W/m$^2$/ano, p$<$0${,}$01). Alexandria (0${,}$13 W/m$^2$/ano) e Macau (0${,}$06 W/m$^2$/ano) demonstraram tendências positivas fracas sem significância. As localidades litorâneas exibiram padrão oposto, com tendências negativas como Nísia Floresta (-0${,}$28 W/m$^2$/ano), Rio do Fogo (-0${,}$20 W/m$^2$/ano), Parnamirim (-0${,}$18 W/m$^2$/ano), Mossoró (-0${,}$12 W/m$^2$/ano) e Monte das Gameleiras (-0${,}$11 W/m$^2$/ano), todas sem significância estatística.

A primavera (SON) apresentou o padrão mais contrastante entre todas as estações, com divergência marcante entre interior e litoral. Santana do Matos manteve tendência positiva altamente significativa (0${,}$52 W/m$^2$/ano, p$<$0${,}$001), acompanhada por Currais Novos (0${,}$48 W/m$^2$/ano, p$<$0${,}$001), Severiano Melo (0${,}$40 W/m$^2$/ano, p$<$0${,}$01) e Alexandria (0${,}$37 W/m$^2$/ano, p$<$0${,}$01). Em contraste acentuado, todas as localidades costeiras e algumas do Seridó apresentaram tendências negativas estatisticamente significativas: Nísia Floresta (-0${,}$32 W/m$^2$/ano, p$<$0${,}$05), Monte das Gameleiras (-0${,}$31 W/m$^2$/ano, p$<$0${,}$05), Rio do Fogo (-0${,}$30 W/m$^2$/ano, p$<$0${,}$05), Mossoró (-0${,}$23 W/m$^2$/ano, p$<$0${,}$01), Macau (-0${,}$23 W/m$^2$/ano, p$<$0${,}$01) e Parnamirim (-0${,}$22 W/m$^2$/ano, p$<$0${,}$05).

\subsection{Precipitação}

A análise das tendências de precipitação acumulada apresentadas nas Tabelas \ref{tab:tendencias_prec_rn_a} e \ref{tab:tendencias_prec_rn_b} revelou padrão predominantemente positivo mas com baixa significância estatística na maioria das localidades analisadas. Esta elevada variabilidade tornou a detecção de mudanças sistemáticas extremamente difícil mesmo em séries temporais médias a longas, particularmente quando as mudanças ocorrem predominantemente através de alterações na frequência de eventos extremos ao invés de mudanças graduais nas médias anuais.
...
O inverno (JJA), período de mínima precipitação no semiárido quando a ZCIT encontra-se em sua posição mais setentrional, apresentou tendências predominantemente positivas em todas as localidades, embora com apenas uma tendência estatisticamente significativa. Monte das Gameleiras, no Seridó, registrou aumento de 3${,}$65 mm/ano (p$<$0${,}$05), representando acréscimo de aproximadamente 128 mm ao longo dos 35 anos analisados. Outras localidades apresentaram tendências positivas expressivas porém sem significância estatística, como Rio do Fogo (4${,}$40 mm/ano), Macau (4${,}$27 mm/ano), Currais Novos (4${,}$14 mm/ano), Severiano Melo (4${,}$04 mm/ano), Nísia Floresta (4${,}$04 mm/ano) e Parnamirim (2${,}$92 mm/ano).

Durante o verão (DJF), as tendências foram predominantemente positivas mas com magnitudes ainda mais modestas e sem qualquer significância estatística. Parnamirim apresentou a tendência mais elevada (1${,}$89 mm/ano), seguida por Rio do Fogo (1${,}$77 mm/ano), Nísia Floresta (1${,}$63 mm/ano), Currais Novos (1${,}$30 mm/ano), Monte das Gameleiras (1${,}$16 mm/ano), Macau (1${,}$09 mm/ano) e Alexandria (0${,}$58 mm/ano). Santana do Matos (0${,}$41 mm/ano) e Severiano Melo (-0${,}$06 mm/ano) exibiram tendências próximas de zero, enquanto Mossoró foi a única localidade com tendência negativa (-1${,}$00 mm/ano), embora sem significância.

O outono (MAM) apresentou padrão altamente heterogêneo e contraditório. Monte das Gameleiras apresentou tendência positiva de 1${,}$39 mm/ano sem significância, enquanto Severiano Melo (0${,}$59 mm/ano), Mossoró (0${,}$40 mm/ano), Macau (0${,}$23 mm/ano), Currais Novos (0${,}$21 mm/ano) e Nísia Floresta (0${,}$08 mm/ano) também exibiram valores positivos modestos. Santana do Matos apresentou tendência praticamente nula (0${,}$06 mm/ano). Em contraste, algumas localidades revelaram tendências negativas como Rio do Fogo (-1${,}$04 mm/ano), Parnamirim (-0${,}$78 mm/ano) e Alexandria (-0${,}$17 mm/ano), todas sem significância estatística.

A primavera (SON), período de transição da estação seca para a chuvosa, apresentou tendências próximas de zero ou ligeiramente negativas em praticamente todas as localidades. Rio do Fogo (0${,}$10 mm/ano), Currais Novos (0${,}$08 mm/ano) e Alexandria (0${,}$04 mm/ano) apresentaram valores positivos desprezíveis, enquanto Monte das Gameleiras (0${,}$01 mm/ano) e Mossoró, Macau e Severiano Melo (todos exatamente 0${,}$00 mm/ano) apresentaram tendências nulas. Parnamirim (-0${,}$06 mm/ano) e Nísia Floresta (-0${,}$17 mm/ano) exibiram tendências ligeiramente negativas sem significância.

A ausência quase total de significância estatística nas tendências de precipitação, mesmo quando as magnitudes foram relativamente elevadas em termos proporcionais aos acumulados típicos, foi consistente com o comportamento altamente esporádico e variável dos eventos precipitantes no semiárido nordestino. A única tendência significativa detectada (Monte das Gameleiras no inverno) representou o aumento em período naturalmente muito seco.

\section{Análise climática no estado do Rio Grande do Norte}
\label{section:analise_rn}

\subsection{Tendências espaciais da temperatura}

A análise espacial das tendências de temperatura do ar no estado do Rio Grande do Norte para o período 1990-2020 (Figura \ref{fig:tend_temp_rn}), apresentando as tendências avaliadas em termos decadais (°C por década), revelou um padrão de aquecimento generalizado embora espacialmente mais heterogêneo que o observado em São Paulo, com taxas de mudança variando significativamente entre diferentes regiões do estado e magnitude média ligeiramente inferior àquela documentada para o território paulista. Este aquecimento, embora menos intenso em termos médios, ocorreu em contexto climático onde as temperaturas absolutas já são elevadas \citep{santos2014caatinga, marengo2017drought}, tornando mesmo mudanças moderadas potencialmente significativas em suas consequências.
...
Durante o verão austral (DJF), estação que no Rio Grande do Norte coincide com o início do período chuvoso no interior semiárido quando a ZCIT atinge sua posição mais ao sul \citep{kousky1980diurnal}, observou-se aquecimento significativo em grande parte do território embora com intensidade variável espacialmente. As taxas de aquecimento situaram-se tipicamente entre 0{,}10 e 0{,}20°C por década conforme a localidade, valores consideravelmente mais heterogêneos que os observados em São Paulo onde o padrão era espacialmente mais uniforme. O interior semiárido apresentou taxas mais elevadas, enquanto a faixa costeira mostrou aquecimento relativamente mais moderado, em torno de 0{,}08 a 0{,}15°C por década.

A região metropolitana de Natal e entorno apresentou padrão com aquecimento moderado (entre 0{,}1 e 0{,}2 °C dependendo da localidade específica), sugerindo que processos de urbanização atuaram, embora sem pontos com significância estatística. A heterogeneidade espacial observada nas tendências de verão pode refletir múltiplos processos incluindo diferenças nas mudanças de uso da terra (particularmente relevantes no semiárido onde conversão de Caatinga em agricultura irrigada ou pastagens pode ter efeitos significativos sobre o balanço de energia superficial) \citep{alkama2016biophysical, leal2003_caatinga}, variações na modulação por sistemas atmosféricos de grande escala, ou simplesmente maior influência da variabilidade natural de escala decadal que, em contexto de séries de trinta anos, pode manifestar-se como aparente heterogeneidade espacial.

O outono (MAM), estação crítica no Rio Grande do Norte por coincidir com o pico da estação chuvosa no interior semiárido quando a ZCIT está tipicamente bem posicionada e ativa sobre o Nordeste brasileiro, manteve padrão de aquecimento em parte do estado, enquanto que a região central e extremo oeste experimentaram tendências negativas mesmo que sem significância estatística. As taxas de aquecimento variaram tipicamente entre 0{,}05 e 0{,}15°C por década, enquanto que as regiões com tendência negativa registraram valores entre -0{,}15 e -0{,}05 °C por década. Esta maior heterogeneidade pode estar associada às características específicas desta estação quando a precipitação exerce papel mais importante na modulação do balanço de energia através da evaporação de água interceptada pela vegetação e do solo.

No inverno (JJA), período que no Rio Grande do Norte coincide com a estação seca no interior quando praticamente não há precipitação e as temperaturas são ligeiramente inferiores ao verão embora ainda elevadas em termos absolutos \citep{alvares2013}, observou-se padrão de aquecimento relativamente consistente em grande parte do território. As taxas variaram tipicamente entre $-0{,}1$ e 0{,}18°C por década, com o interior continental mostrando ligeira tendência a valores mais elevados que a faixa costeira.

A primavera (SON) destacou-se como apresentando algumas das tendências de temperatura mais positivas e espacialmente coerentes de todo o ciclo anual, com aquecimento significativo em praticamente todo o território estadual, particularmente no litoral norte. As taxas de aquecimento primaveril situaram-se frequentemente entre 0{,}1 e 0{,}25°C por década em diversas localidades, com algumas áreas mostrando significância estatística conforme indicado pelas hachuras.

A integração dos padrões sazonais sugeriu que o estado do Rio Grande do Norte experimentou aquecimento médio da ordem de 0{,}3 a 0{,}6°C ao longo do período 1990-2020 dependendo da região considerada, com algumas áreas específicas podendo ter experimentado incrementos ligeiramente superiores. Embora estes valores tenham sido inferiores aos observados em São Paulo, eles ocorreram em contexto onde as temperaturas absolutas são substancialmente mais elevadas (médias anuais de 26 a 28°C comparadas com 18 a 23°C em São Paulo) \citep{alvares2013}. Neste contexto, mesmo aquecimento moderado pode ter consequências desproporcionalmente importantes ao empurrar temperaturas para além de limiares críticos de tolerância fisiológica ou ao intensificar processos dependentes de temperatura como evapotranspiração em situação já caracterizada por déficit hídrico considerável como o observado para a região nordeste \citep{marengo2017drought}.

\subsection{Tendências espaciais da precipitação}

As tendências de precipitação no estado do Rio Grande do Norte (Figura \ref{fig:tend_prec_rn}) apresentaram padrão ainda mais heterogêneo e espacialmente fragmentado que o observado em São Paulo, com pouquíssimas regiões apresentando significância estatística.
...
No intervalo compreendido em (DJF), estação que marca o início da estação chuvosa no interior semiárido quando a ZCIT começa a migrar para sul \citep{kousky1980diurnal}, observou-se padrão fragmentado de tendências estatisticamente significativas. Algumas localidades apresentaram tendências fracas positivas, outras mostraram tendências fracas negativas, e partes do centro e litoral apresentaram tendências positivas de acumulado de precipitação, mas com poucas hachuras de significância. As magnitudes das tendências observadas (quando estatisticamente detectáveis) foram tipicamente inferiores a $\pm$10~mm por década.

O outono (MAM), estação crítica do ponto de vista hidrológico por concentrar a maior parte da precipitação anual no interior semiárido (tipicamente 50 a 60\% do total anual ocorre durante MAM) \citep{velloso2002_ecorregioes_caatinga}, apresentou um padrão mais homogêneo, mas sem sinais de significância. Esta ausência de tendências estatisticamente significativas durante a estação mais importante do ciclo hidrológico anual poderia ser interpretada como indicação de relativa estabilidade do regime pluviométrico principal, porém ao lidarmos com a variável de precipitação a variabilidade deve ser levada em consideração, além disso os padrões orográficos distribuídos por todo o estado que podem modificar o perfil de precipitação.

No inverno (JJA), os resultados indicaram uma escassez extrema de precipitação, com regiões com tendências negativas no litoral que indicaram valores abaixo do esperado para aquele período. De modo geral, pontos do estado demonstraram tendências ligeiramente acima da média, no entanto boa parte ficou em torno da média e regiões próximas à capital Natal tiveram resultados abaixo da média, mas para todo o estado não foram observadas significâncias estatísticas.

A primavera (SON), estação que marca a transição entre o período seco e o início do retorno das chuvas, apresentou todo o estado com valores em torno da média e alguns poucos pontos com significância estatística. Observou-se pequena área com tendências positivas estatisticamente significativas próximas ao centro do estado, mas sutil ao ponto de se confundir com valores alinhados à média.

\subsection{Tendências espaciais de radiação solar}

A radiação solar incidente na superfície no estado do Rio Grande do Norte (Figura \ref{fig:tend_rad_rn}) apresentou tendências predominantemente positivas em diversas regiões do estado, com padrão espacial relativamente mais organizado que o observado para precipitação embora ainda apresentando heterogeneidade significativa.
...
As tendências de radiação solar observadas para o conjunto correspondente ao verão (DJF) foram predominantemente positivas, com incrementos típicos entre 3 e 8 W/m$^2$ por década conforme a localidade. Grande parte do território apresentou tendências positivas estatisticamente significativas, particularmente no interior semiárido e na porção oeste do estado, como evidenciado pelas extensas áreas hachuradas na figura.

O outono (MAM) apresentou tendências predominantemente positivas e espacialmente consistentes, com magnitudes frequentemente superiores a 8 W/m$^2$ por década em grande parte do território e ampla significância estatística. Este incremento substancial durante a principal estação chuvosa foi particularmente notável e pode estar relacionado a mudanças nos padrões de nebulosidade convectiva ou a alterações na composição atmosférica que afetam a transmissividade radiativa \citep{wild2012}.

O inverno (JJA) apresentou tendências positivas mais consistentes e intensas de todo o ciclo anual, com incrementos frequentemente superiores a 8 W/m$^2$ por década em praticamente todo o estado, e diversas localidades alcançando valores próximos a 10 W/m$^2$ por década. A extensão espacial das áreas com significância estatística foi particularmente notável durante esta estação, abrangendo quase todo o território estadual.

A primavera (SON) manteve tendências predominantemente positivas com magnitudes moderadas a elevadas, tipicamente entre 5 e 10 W/m$^2$ por década, com significância estatística em parcela substancial do território. O padrão espacial mostrou certa heterogeneidade, com o interior semiárido apresentando os incrementos mais elevados.

Apesar das tendências sazonais de radiação indicarem em boa parte significância estatísica, ao compararmos o perfil demonstrado na Figura \ref{fig:tend_rad_rn} em relação à Figura \ref{fig:tend_prec_rn}, foi possível identificar que mesmo em períodos mais chuvosos, como o outono (MAM) isso não impediu com que os valores de radiação fossem consideravelmente altos, especilamente nas regiões situadas entre a parte central e extremo oeste do estado do Rio Grande do Norte. Pensando nisto, adotou-se uma investigação à série temporal de precipitação horária acumulada com o objetivo de identificar em quais períodos foram registradas maiores regimes pluviométricos para essa porção. 

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{fig/ciclo_diurno_por_estacao.png}
    \caption{Série temporal de precipitação horária acumulada concentrada por estação do ano, entre a região central e Extremo Oeste do Rio Grande do Norte para o período 2000--2024.}
    \label{fig:ciclo_diurno_estacoes}
\end{figure}

Com base na Figura \ref{fig:ciclo_diurno_estacoes}, foi possível observar que, em todas as estações, o período da manhã (06h--12h) apresentou consistentemente os menores volumes acumulados, enquanto o período noturno e da madrugada concentrou as maiores frações da precipitação total.

Durante o outono (MAM), estação que concentra a maior parte da precipitação anual no semiárido \citep{velloso2002_ecorregioes_caatinga}, esse perfil é particularmente pronunciado: os volumes das primeiras horas da 
madrugada (00h--02h) superam em mais de uma ordem de grandeza os registrados entre 06h e 12h, e o pico noturno entre 18h e 22h atinge os maiores valores absolutos de todo o ciclo anual. No verão (DJF), 
embora a distribuição seja ligeiramente mais uniforme, o mínimo matutino persiste e o período noturno ainda responde pela maior fração do volume total. No inverno (JJA), apesar dos volumes globalmente reduzidos em razão da escassez de precipitação característica da estação seca \citep{kousky1980diurnal}, o padrão se mantém com máximos entre 18h e 20h e mínimos bem definidos na faixa das 08h--12h. A primavera (SON), período de transição com volumes muito baixos, reproduz a mesma estrutura, com a madrugada respondendo pela parcela 
mais expressiva do escasso total precipitado.

\section{Implicações da cobertura de superfície no clima}

A análise das mudanças climáticas apresentadas nas seções anteriores quantificou e qualificou as variações nos últimos 35 anos das variáveis climáticas abordadas neste trabalho. Porém, ao observarmos de forma integrada, podemos perceber que em alguns pontos dos dois estados existem divergências na comparação conjunta dessas três variáveis. A exemplo disso, podemos destacar a região sul do estado de São Paulo, onde estão localizados municípios como Sete Barras, Eldorado e Registro, distribuídos na região do Vale do Ribeira, área conhecida por seu remanescente de Mata Atlântica \citep{ribeiro2009_atlanticforest, sos_inpe_atlas}. Porém, ao observarmos a Figura \ref{fig:tend_prec_sp}, o que se destaca principalmente no período de verão austral (DJF) é que, além de ser um dos poucos pontos com \textit{p}-valor de significância estatística consistente (abaixo de 0,05), a região apresentou uma tendência negativa considerável, com valores variando entre $-7{,}5$ e $-10{,}0$~mm/ano. No entanto, nas demais análises de tendência (temperatura e radiação), a região não apresentou comportamentos drásticos, ou seja, aumento considerável de temperatura e radiação. Sendo assim, algumas perguntas surgem, em especial: se a precipitação acumulada diminuiu, por que a temperatura e a radiação incidente na superfície não aumentaram de forma proporcionalmente inversa à redução da precipitação?

Pensando em uma resposta para esta pergunta, propõe-se, portanto, uma avaliação da influência da cobertura de superfície nos dois estados, ou seja, avaliar se o fato de ter mais vegetação é capaz de absorver parte da radiação incidente de onda curta e, por consequência, diminuir a de onda longa reemitida para a atmosfera, sendo esta uma das responsáveis pelo aumento de temperatura \citep{alkama2016biophysical, duveiller2018local}. Portanto, esta seção integra sistematicamente os dados climáticos anteriormente apresentados com informações detalhadas sobre mudanças no uso e cobertura da terra derivadas do projeto MapBiomas \citep{souza2020_mapbiomas, mapbiomas_collection8_platform}, permitindo a identificação de correlações espaciais e temporais que sugerem relações causais entre transformações ambientais e mudanças climáticas observadas. A análise espacial foi feita considerando células de 100$\times$100~km por cobrir espacialmente as grades definidas por todas as variáveis climáticas (Figura \ref{fig:mapbiomas_sp_1990}), o processo considerou a representação de cores apresentada pela Tabela \ref{tab:mapbiomas_classes} que descreve a tipagem de cobertura segundo a classificação feita via pacote de dados MapBiomas Collection, via QGIS.
...
\caption[Uso e cobertura da terra no estado de São Paulo em 1990 segundo classificação MapBiomas, Collection 9]{Uso e cobertura da terra no estado de São Paulo em 1990 segundo classificação MapBiomas, Collection 9.}
...
Já em 2020 o que se revelou foram transformações marcantes nesta configuração. Observou-se intensificação agrícola substancial, com expansão de cultivos de cana-de-açúcar (tons rosa-lilás) nas regiões norte, noroeste e centro-oeste, ocupando áreas anteriormente destinadas a pastagens. A conversão de pastagens de baixa produtividade em agricultura mecanizada foi particularmente evidente nas regiões de Ribeirão Preto, Barretos e Araçatuba. Simultaneamente, as formações florestais na Serra do Mar e Serra da Mantiqueira apresentaram aparente expansão ou adensamento (tons verdes mais intensos) \citep{sos_inpe_atlas_2023_2024}. A mancha urbana apresentou expansão moderada mas geograficamente concentrada, com crescimento mais visível na região metropolitana de São Paulo e no entorno de cidades médias do interior, representada pela intensificação dos tons avermelhados.

O estado do Rio Grande do Norte, por sua vez, apresentou transformações ainda mais dramáticas (Figura \ref{fig:mapbiomas_rn_1990}). As alterações observadas caracterizaram um dos processos de conversão de vegetação nativa mais severos documentados no Brasil \citep{leal2003_caatinga, velloso2002_ecorregioes_caatinga}.

\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{fig/mapa_grade_rn.png}
\caption[Uso e cobertura da terra no estado do Rio Grande do Norte em 1990 segundo classificação MapBiomas, Collection 9]{Uso e cobertura da terra no estado do Rio Grande do Norte em 1990 segundo classificação MapBiomas, Collection 9.}
...
O painel de 2020 apresentou uma transformação particularmente drástica desta configuração original. A cobertura de Caatinga sofreu redução significativa e espacialmente generalizada, com conversão massiva em pastagens degradadas (tons amarelados agora predominantes em vastas áreas do interior), agricultura de sequeiro de baixa produtividade, solo exposto e áreas em diversos estágios de degradação ambiental \citep{santos2014caatinga, sudene2024desertificacao}. Esta conversão foi particularmente severa e visualmente impactante nas regiões centro-norte e central do estado, onde a vegetação nativa característica (tons verdes) foi substituída quase completamente por tons amarelados. Os remanescentes de Caatinga preservada (tons verdes) restringiram-se a fragmentos dispersos e áreas específicas no sul do estado e em localidades pontuais.

A região costeira apresentou expansão visível de áreas urbanizadas, especialmente em Natal, Mossoró e ao longo do litoral oriental (intensificação dos tons avermelhados), e intensificação marcante da aquicultura com ampliação de manchas lilás. O contraste visual entre os dois painéis foi extremamente marcante: o que em 1990 era um estado predominantemente coberto por vegetação nativa semiárida (tons verdes ocupando a maior parte do território) transformou-se em 2020 em território dominado por usos antrópicos de baixa produtividade e cobertura vegetal severamente degradada (tons amarelados).

\subsection{Análise das mudanças nas classes de uso da terra}

A quantificação das transformações no uso e cobertura da terra permite avaliar a magnitude absoluta e relativa das mudanças observadas nos mapas qualitativos apresentados anteriormente. As Figuras \ref{fig:sp_top10_classes} e \ref{fig:rn_top10_classes} apresentam as dez classes de uso da terra mais representativas em cada estado nos anos de 1990 e 2020, enquanto as Figuras \ref{fig:sp_variacao_classes} e \ref{fig:rn_variacao_classes} quantificaram as variações absolutas (em pontos percentuais) experimentadas por cada classe ao longo do período de três décadas.
...
A análise quantitativa para São Paulo (Figura \ref{fig:sp_top10_classes}) revelou transformação profunda na estrutura de uso da terra. Em 1990, as pastagens dominavam o território do estado de São Paulo, ocupando 37,4\% da área total, seguidas por formação florestal (17,7\%) e mosaico de usos (13,9\%). Em 2020, embora as pastagens permanecessem como a classe mais representativa, sua extensão reduziu-se dramaticamente para 22,1\%, representando perda absoluta de 15,3 pontos percentuais. Esta redução massiva de pastagens foi compensada principalmente pela expansão de agricultura intensiva: cana-de-açúcar expandiu-se de 3,5\% para 12,1\%, soja de praticamente zero para 8,1\%, e formação campestre de 1,2\% para 4,3\%.
...
A análise quantitativa para o Rio Grande do Norte (Figura \ref{fig:rn_top10_classes}) revelou transformação ainda mais dramática que a observada em São Paulo, caracterizada pela diminuição da vegetação nativa de Caatinga. Em 1990, a formação savânica dominava o estado com 27,0\% do território, seguida por pastagens (10,2\%) e mosaico de usos (9,4\%). Em 2020, a formação savânica reduziu-se para 23,6\%, representando perda absoluta de 3,4 pontos percentuais da cobertura nativa. Simultaneamente, as pastagens expandiram-se moderadamente de 10,2\% para 11,4\% (+1,2\%), e particularmente notável foi a emergência de ``outras lavouras temporárias'' que expandiram dramaticamente de 0,2\% para 3,7\% (+3,5\%).
...
A Figura \ref{fig:rn_variacao_classes} quantificou estas transformações de forma detalhada. A formação savânica (Caatinga) apresentou a maior redução absoluta ($-3{,}4$\%), seguida por mosaico de usos ($-2{,}3$\%) e formação florestal ($-0{,}3$\%). Em contraste, ``outras lavouras temporárias'' apresentaram o maior ganho absoluto ($+3{,}5$\%), seguidas por pastagens ($+1{,}2$\%), outras lavouras perenes ($+0{,}9$\%) e área urbanizada ($+0{,}6$\%). O padrão geral indicou processo de conversão extensiva de vegetação nativa de Caatinga em usos antrópicos de baixa produtividade, particularmente pastagens degradadas e agricultura de sequeiro, caracterizando um dos processos de degradação ambiental mais severos documentados no semiárido brasileiro \citep{leal2003_caatinga, sudene2024desertificacao}.
...
Ao analisar a Figura \ref{fig:mapa_blocos_extremos_sp} o que se destacou foi: a evolução temporal da cobertura de formação florestal (predominantemente Mata Atlântica) apresentou duas trajetórias distintas que refletiram diferentes contextos espaciais. Em áreas sob maior pressão de conversão agrícola e urbana, a cobertura florestal reduziu-se de 15,0\% em 1990 para 6,0\% em 2020, representando perda absoluta de 9,0 pontos percentuais.

Avaliando progressivamente o comportamento de 10 em 10 anos, o que se observou foi uma queda progressiva nos últimos anos, estabilizando-se de 2010 a 2020, Figura \ref{fig:evolucao_cobertura_sp}.
...
A análise integrada das duas figuras evidenciou, portanto, que as mudanças na cobertura florestal paulista não seguiram padrão estadual homogêneo. As perdas concentraram-se em regiões de agricultura intensiva no interior (noroeste), enquanto ganhos ocorreram em áreas de proteção ambiental e topografia acidentada (Serra do Mar) \citep{sos_inpe_atlas_2023_2024}.

\subsection{Avaliação e extremos de cobertura de solo no estado do Rio Grande do Norte}

A análise espacial (Figura \ref{fig:mapa_blocos_extremos_rn}) revelou alta heterogeneidade, com células individuais apresentando perdas de até $-43{,}4\%$ e ganhos de até $+14{,}0\%$ de cobertura de Caatinga entre 1990 e 2020. O mapa evidenciou padrão espacial complexo: áreas de maior perda (tons vermelho-escuro) concentraram-se predominantemente na região centro-norte do estado (célula com perda máxima de $-43{,}4\%$ localizada em latitude $-5^\circ$ e longitude $-37^\circ$, correspondendo aproximadamente à região do Seridó), enquanto áreas de ganho ou menor mudança (tons verdes) distribuíram-se principalmente no sul e sudoeste (célula com ganho máximo de $+14{,}0\%$ em latitude $-6{,}5^\circ$ e longitude $-38^\circ$). A região central e leste apresentou mosaico heterogêneo de perdas moderadas a severas (tons alaranjados a vermelhos).
...
A Figura \ref{fig:evolucao_cobertura_rn} revelou as trajetórias temporais que caracterizaram estas regiões extremas. A linha vermelha representa áreas sob maior pressão antrópica, onde a cobertura de Caatinga sofreu redução de 79,0\% em 1990 para aproximadamente 68,0\% em 2000, seguida por nova queda para 60,0\% em 2010, decaindo progressivamente para 35,5\% em 2020. Esta trajetória evidenciou processo de degradação acelerada, particularmente severa na última década (2010-2020), quando ocorreu a maior taxa absoluta de perda ($-24{,}5\%$ em apenas 10 anos).
...
Em contraste marcante, a linha verde representa áreas com trajetória de recuperação ou preservação relativa, iniciando em 56,6\% de cobertura em 1990 e apresentando crescimento progressivo para 62,0\% (2000), 73,0\% (2010), com leve redução para 70,6\% (2020), resultando em ganho de 14{,}0\% ao longo do período. Foi observado um decaimento considerável da perda savânica após 2010 (linha vermelha: queda de 60\% para 35,5\%) que contrastou com relativa estabilização nas áreas menos degradadas (linha verde: pequena redução de 73\% para 70,6\%).

A análise integrada das duas figuras evidenciou que o Rio Grande do Norte experimentou processo de diferenciação espacial extrema, com áreas apresentando recuperação de cobertura vegetal, enquanto a maioria do território sofreu degradação progressiva quando observado em conjunto com a Figura \ref{fig:mapa_blocos_extremos_rn} que demonstrou uma mudança considerável nos dois cenários.

\section{Variáveis climáticas em áreas de extremos de mudança de cobertura}

A análise das variáveis climáticas em blocos de grade apresentadas no decorrer desta seção pelas Figuras \ref{fig:mapa_blocos_extremos_sp} e \ref{fig:mapa_blocos_extremos_rn} com a descrição das localidades das mudanças extremas de cobertura vegetal permite avaliar se as variações de cobertura estão associadas aos padrões climáticos analisados nas seções anteriores. Para cada estado, selecionaram-se quatro blocos representando os extremos: os dois blocos com maior perda de vegetação nativa versus os dois blocos com maior ganho ou menor perda de vegetação. As médias anuais de temperatura e radiação solar e acumulados de precipitação foram calculadas para estes quatro blocos ao longo do período 1990--2024.

\subsection{São Paulo: Formação Florestal}

\subsubsection*{Temperatura}

A Figura \ref{fig:temp_extremos_sp} apresenta as médias anuais de temperatura para os quatro blocos extremos de mudança de formação florestal no estado de São Paulo. Observou-se clara e persistente separação térmica entre os dois grupos ao longo de todo o período analisado. Os dois blocos com maior perda de vegetação florestal (linhas vermelhas, sólida e tracejada) apresentaram temperaturas consistentemente elevadas, oscilando entre 24,0 e 26,5°C ao longo da série temporal, com média aproximada de 24,8°C. Em contraste marcante, os dois blocos com ganho ou menor perda florestal (linhas verdes, sólida e tracejada) mantiveram-se consistentemente mais frios, variando entre 17,5 e 20,5°C, com média próxima a 19,0°C.
...
A diferença entre os dois grupos manteve-se notavelmente constante em aproximadamente 5,0 a 5,5°C ao longo de três décadas e meia. As trajetórias históricas dos dois blocos de cada grupo apresentaram comportamento coerente entre si, com as linhas vermelhas (Perda \#1 e Perda \#2) seguindo trajetórias praticamente paralelas, assim como as linhas verdes (Ganho \#1 e Ganho \#2), reforçando o padrão observado. Esta magnitude e persistência da diferença térmica sugeriram que ela era primariamente determinada por fatores geográficos (altitude, latitude) \citep{alvares2013}, ao invés de mudanças dinâmicas na cobertura vegetal. Os blocos com maior perda florestal localizaram-se predominantemente no interior noroeste do estado, em altitudes baixas (300--500m) e latitudes menores, enquanto blocos com ganho ou menor perda concentraram-se na Serra do Mar, em altitudes elevadas (800--1200m) e latitude ligeiramente maior.

Ambos os grupos apresentaram tendência clara de aquecimento ao longo do período, particularmente visível após 2010. Blocos com maior perda florestal mostraram aquecimento de aproximadamente 1,5 a 2,0°C entre 1990 e 2024, enquanto blocos preservados apresentaram aquecimento de magnitude similar (1,0 a 1,5°C). A consistência das taxas de aquecimento entre os dois grupos, independentemente da magnitude da mudança de cobertura florestal, sugeriu que o aquecimento regional era controlado primariamente por forçantes climáticas de grande escala \citep{ipcc2021} ao invés de mudanças locais na cobertura vegetal.

\subsubsection*{Radiação Solar}

A Figura \ref{fig:rad_extremos_sp} evidenciou comportamento de radiação solar incidente fundamentalmente distinto entre os dois grupos de blocos. Os dois blocos com maior perda florestal (linhas vermelhas) apresentaram valores consistentemente elevados e trajetórias altamente coerentes entre si, oscilando entre 205 e 238~W/m$^2$ ao longo da série temporal, com média aproximada de 220~W/m$^2$. Os dois blocos preservados (linhas verdes) mantiveram radiação substancialmente inferior, também com alta coerência interna, variando entre 170 e 220~W/m$^2$, com média próxima a 190~W/m$^2$.
...
A diferença radiativa entre os grupos permaneceu aproximadamente constante em 30 a 40~W/m$^2$ ao longo do período, representando redução de cerca de 15 a 20\% na radiação disponível nas áreas preservadas comparadas às áreas com maior perda florestal. A alta coerência temporal entre os dois blocos de cada grupo (linhas praticamente paralelas dentro de cada cor) demonstrou que esta diferença era geograficamente determinada e persistente. Regiões preservadas na Serra do Mar apresentavam maior nebulosidade orográfica devido ao levantamento de massas de ar úmidas oceânicas, enquanto blocos do interior noroeste beneficiavam-se de maior transparência atmosférica e menor nebulosidade média, resultando em maior incidência de radiação sobre a superfície \citep{yamasoe2017}.

Ambos os grupos apresentaram tendência de aumento da radiação solar ao longo do período, particularmente evidente após 2015. A taxa de aumento foi similar entre os grupos (aproximadamente 15 a 20~W/m$^2$ ao longo de 34 anos), com ambas as linhas de cada grupo seguindo trajetórias ascendentes paralelas, sugerindo que esta tendência de \textit{brightening} \citep{wild2012} era controlada por processos atmosféricos regionais ao invés de mudanças locais na cobertura vegetal.

\subsubsection*{Precipitação}

A Figura \ref{fig:prec_extremos_sp} apresentou diferenças marcantes de precipitação acumulada entre os dois grupos de blocos. Os dois blocos com ganho ou menor perda florestal (linhas verdes), localizados predominantemente na Serra do Mar, apresentaram precipitação acumulada anual consistentemente superior e trajetórias altamente coerentes entre si, oscilando entre 60.000 e 110.000~mm acumulados, com média próxima a 85.000~mm. Em contraste, os dois blocos com maior perda florestal (linhas vermelhas), situados no interior noroeste, apresentaram valores substancialmente inferiores e também alta coerência interna, variando entre 50.000 e 80.000~mm, com média próxima a 65.000~mm.
...
A diferença de precipitação entre os grupos manteve-se relativamente constante ao longo do período, oscilando entre 15.000 e 30.000~mm dependendo do ano específico, equivalente a aproximadamente 20 a 35\% a mais de precipitação nas áreas preservadas. A alta coerência entre os dois blocos de cada grupo (trajetórias praticamente paralelas dentro de cada cor) demonstrou que esta diferença era novamente geograficamente determinada. Esta diferença foi controlada por efeitos como: blocos preservados na Serra do Mar beneficiavam-se de intensificação pluviométrica devido ao levantamento forçado de massas de ar úmidas oceânicas que ascendiam as encostas da serra, experimentavam resfriamento adiabático, condensação e precipitação intensificada, particularmente nas vertentes voltadas para o oceano \citep{alvares2013}.

Observou-se tendência sutil de redução de precipitação em ambos os grupos após 2015, com os blocos de maior perda florestal apresentando declínio ligeiramente mais significativo. Porém, a interpretação destas diferenças de precipitação requer um aprofundamento na análise: embora blocos preservados recebessem substancialmente mais precipitação, esta diferença refletia primariamente localização geográfica (Serra do Mar versus interior) ao invés de efeitos diretos da cobertura vegetal \citep{li2015local}. As mesmas características topográficas que induziram alta precipitação também favoreceram historicamente a preservação florestal, criando correlação geográfica que não implica necessariamente causalidade. Efeitos da vegetação sobre precipitação local através de evapotranspiração e ciclagem de umidade \citep{eagleson1978, Trenberth2009} podem existir.

\subsection{Rio Grande do Norte: Formação Savânica}

\subsubsection*{Temperatura}

A Figura \ref{fig:temp_extremos_rn} apresenta as médias anuais de temperatura para os quatro blocos extremos de mudança de Caatinga no Rio Grande do Norte. Em contraste marcante com São Paulo, observou-se que os quatro blocos apresentaram temperaturas altamente convergentes ao longo de todo o período analisado, com as quatro linhas frequentemente entrelaçadas e sobrepondo-se umas às outras. Os dois blocos com maior redução de Caatinga (linhas vermelhas) oscilaram entre valores próximos a 27,0 e 29,0°C, enquanto os dois blocos com ganho ou menor redução (linhas verdes) variaram entre valores um pouco abaixo de 27,0 e máximo por volta de 28,5°C, com todas as quatro linhas convergindo repetidamente ao longo da série temporal.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{fig/temp_formacao_savanica_rn.png}
...
A diferença térmica média entre os grupos foi mínima, tipicamente inferior a 0,5°C, e não apresentou consistência temporal. A ausência de diferenciação clara entre os quatro blocos, independentemente da magnitude da mudança de cobertura vegetal, contrastou fortemente com o padrão observado em São Paulo. A variabilidade interanual superou amplamente qualquer possível diferença sistemática entre áreas preservadas e degradadas, dificultando a identificação de sinais associados especificamente a mudanças na cobertura de Caatinga. A análise visual sugeriu possível tendência sutil de aquecimento ao longo do período em todos os blocos, consistente com tendências regionais apresentadas anteriormente \citep{santos2014caatinga}, mas sem diferença detectável nas taxas de aquecimento entre blocos preservados e degradados.

\subsubsection*{Radiação Solar}

A Figura \ref{fig:rad_extremos_rn} apresentou padrão notavelmente distinto daquele observado em São Paulo. Os quatro blocos (dois de maior perda de Caatinga em vermelho e dois de ganho ou menor perda em verde) apresentaram valores de radiação solar altamente convergentes, com as quatro linhas entrelaçando-se e sobrepondo-se substancialmente ao longo da série temporal. Todos os blocos oscilaram em faixas que se sobrepuseram amplamente, variando entre aproximadamente 245 e 277~W/m$^2$, sem diferença sistemática consistente entre blocos de perda e blocos de ganho.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{fig/rad_formacao_savanica_rn.png}
...
A alta variabilidade interanual observada em todos os blocos (oscilações de até 30 a 35~W/m$^2$ entre anos consecutivos) superou amplamente qualquer diferença média potencial entre os grupos, dificultando a identificação de sinais associados especificamente a mudanças na cobertura vegetal \citep{alkama2016biophysical, duveiller2018local}. O entrelaçamento constante das quatro linhas ao longo de toda a série temporal demonstrou que blocos com históricos radicalmente distintos de mudança de cobertura (perda severa versus ganho substancial) experimentaram condições radiativas essencialmente indistinguíveis. Esta variabilidade esteve provavelmente associada a flutuações na nebulosidade durante a estação chuvosa \citep{medeiros2017}, que variou de ano para ano dependendo da intensidade e posicionamento da Zona de Convergência Intertropical \citep{kousky1980diurnal}.

\subsubsection*{Precipitação}

A Figura \ref{fig:prec_extremos_rn} apresentou o comportamento da precipitação acumulada com características específicas do semiárido. Os quatro blocos apresentaram precipitação total anual variando entre aproximadamente 3.000 e 30.000~mm, com variabilidade interanual extremamente elevada que dominou completamente qualquer diferença média potencial entre os grupos. As quatro linhas entrelaçaram-se de forma consistente ao longo da série, com alternância frequente de posições entre blocos de perda e blocos de ganho.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{fig/prec_formacao_savanica_rn.png}
...
O coeficiente de variação interanual para boa parte dos blocos excedeu 60\%, valores altos mesmo para padrões semiáridos \citep{marengo2017drought, marengo2017}. Eventos de seca severa (precipitação $<$5.000~mm) alternaram-se com anos relativamente úmidos ($>$25.000~mm) sem padrão previsível. Esta alta variabilidade dificultou a identificação de possíveis diferenças entre áreas preservadas e degradadas.

Os dois blocos com ganho ou menor perda de Caatinga (linhas verdes) apresentaram tendência sutil de precipitação ligeiramente superior em alguns anos, particularmente visível nos picos de 2011 (~30.000~mm) e 2024 (~26.000~mm), mas com os perfis em vermelho, especialmente a Perda \#2, acompanhando de forma persistente o perfil observado para os casos de ganho. No entanto, esta diferença não foi consistente ao longo da série e pode refletir simplesmente localização geográfica dos blocos, uma delas localizada em uma região com altitudes ultrapassando 400 m (principalmente para o Ganho \#2), enquanto que o Ganho \#1 recebe influência direta do Oceano no litoral centro-norte. A convergência frequente das quatro linhas em anos secos (particularmente 2012--2013 e 2016--2017) demonstrou que em condições de seca severa, controlada por circulação atmosférica de grande escala, possíveis efeitos locais da vegetação tornaram-se irrelevantes.
...
Entre 1990 e 2020, observou-se conversão substancial de pastagens (tons amarelados) em cana-de-açúcar (tons rosa-lilás) nas regiões norte, noroeste e central do estado. Simultaneamente, a Serra do Mar manteve ou intensificou tonalidade verde, consistente com preservação ou recuperação de cobertura florestal.

As variáveis climáticas (linhas 2-4) mantiveram padrões espaciais notavelmente estáveis ao longo dos 30 anos. A precipitação preservou gradiente litoral-interior característico em todos os períodos. A temperatura mostrou gradiente topográfico estável (Serra do Mar mais fria, interior mais quente) que se manteve praticamente inalterado apesar das transformações de uso da terra. A radiação solar manteve padrão inverso ao da precipitação (máximos no interior, mínimos no litoral) de forma consistente nos quatro períodos.
...
Entre 1990 e 2020, observou-se conversão substancial de áreas verdes (Caatinga preservada) em tons amarelados e acastanhados (pastagens degradadas, solo exposto), particularmente intensa nas regiões centro-norte e central do estado.

As linhas de precipitação (segunda linha) e temperatura (terceira linha) não apresentaram padrões espaciais de mudança claramente associados às áreas de maior degradação de Caatinga. A precipitação manteve gradiente leste-oeste característico (litoral úmido, interior seco) em todos os quatro períodos, com variações interanuais que refletiram primariamente flutuações na intensidade da estação chuvosa ao invés de mudanças sistemáticas associadas a uso da terra. A temperatura mostrou gradiente norte-sul relativamente estável ao longo do período, com interior norte mais quente (27-28 °C) e litoral sul ligeiramente mais frio (26-27 °C).

Já para a radiação solar representada na última linha, observou-se um padrão espacial complexo que variou substancialmente entre os anos analisados. Nos anos de 1990 e 2000, os perfis foram contrários aos apresentados para temperatura e precipitação no mesmo período. Já para os anos subsequentes, o que se observou foi uma concordância, em especial para o ano de 2020, com valores mais consideráveis entre as regiões Central e Oeste Potiguar. Essa concordância apresentou, contudo, uma exceção em pontos onde estão localizados municípios como Luís Gomes e São Miguel, na região popularmente conhecida como a ``Tromba do Elefante'' \citep{fernandes2016}\footnote{Neste trabalho de dissertação são apresentados os limites estaduais para o estado do Rio Grande do Norte e como esta formação espacial é curiosamente caracterizada pela silhueta/contorno de um elefante. A construção histórica do estado mostra como se tornou popular no cotidiano potiguar reconhecer a região do Alto Oeste como a região da \textit{tromba do elefante}.}.

Esse extremo oeste do estado configura-se como uma ilha de excepcionalidade climática em relação ao contexto geral do semiárido potiguar, o que se justifica pela presença dos maciços residuais do sudoeste potiguar, com altitudes que variam em torno de 600 metros, entre os quais se destacam as serras de São Miguel, Martins e Luís Gomes. O relevo mais elevado favorece a ascensão forçada das massas de ar úmido, mecanismo denominado efeito orográfico, promovendo condensação e, consequentemente, maior frequência de precipitação e cobertura de nuvens na região \cite{silveira2016}.

Essa condição é reforçada pelo fato de a Serra de Luís Gomes ser classificada como brejo de altitude, uma das quatro áreas de exceção climática identificadas no Rio Grande do Norte \cite{medeiros2019}. Os brejos de altitude são caracterizados por vegetação mais densa e úmida em meio ao semiárido, o que intensifica o processo por meio da evapotranspiração, incrementando a umidade atmosférica local e retroalimentando tanto a formação de nuvens quanto o arrefecimento térmico da superfície \cite{silveira2016}. Esse mecanismo explica, em particular, a acentuação do padrão observado a partir de 2010, quando os menores valores de radiação solar passaram a se concentrar de forma mais consistente nessa porção do estado.

\subsection{Estudo de caso: o Extremo Oeste Potiguar como área de exceção climática}

Após a análise feita para o estado do Rio Grande do Norte, foi possível observar um comportamento climático singular no extremo oeste do estado em destaque na Figura \ref{fig:mapas_temporais_rn}, correspondente à região conhecida como a ``Tromba do Elefante'' \citep{fernandes2016}, onde se localizam municípios como Luís Gomes, São Miguel e Martins. Para investigar mais detalhadamente este padrão, realizou-se uma análise comparativa entre essa sub-região e o estado do Rio Grande do Norte como um todo, avaliando a cobertura de Formação Savânica e Área Urbana em relação às variáveis climáticas para os anos de 1990, 2000, 2010 e 2020.

\subsubsection*{Formação Savânica}

A Figura~\ref{fig:savana_extremo_oeste_vs_rn} apresenta a comparação entre a cobertura de Formação Savânica (Caatinga) e as variáveis climáticas para o Extremo Oeste do RN e o estado. O Extremo Oeste manteve cobertura savânica consistentemente superior à média estadual ao longo de todo o período, variando entre 68{,}8\% (1990) e 69{,}5\% (2020), com ligeiro pico de 72{,}4\% em 2010. Em contraste, o estado do Rio Grande do Norte apresentou redução contínua, de 53{,}4\% em 1990 para 46{,}4\% em 2020, evidenciando a maior preservação relativa da vegetação nativa na sub-região serrana.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{fig/fig1_savana_extremo_oeste_vs_rn.png}
    \caption[Cobertura de Formação Savânica e variáveis climáticas: Extremo Oeste do RN \textit{versus} 
    estado do RN (1990--2020)]{Comparação entre a cobertura de Formação Savânica (Caatinga) 
    e as variáveis climáticas temperatura média, precipitação total e radiação solar média para 
    o Extremo Oeste do RN e o estado como um todo nos anos de 1990, 2000, 2010 e 2020. 
    Valores em verde referem-se ao Extremo Oeste; valores em azul ao estado do RN.}
    \label{fig:savana_extremo_oeste_vs_rn}
\end{figure}

Do ponto de vista climático, a sub-região apresentou precipitação média superior à média estadual em todos os períodos analisados, com diferença que se ampliou ao longo do tempo: de aproximadamente 90~mm em 1990 (287{,}7~mm no estado \textit{versus} 379{,}5~mm no Extremo Oeste) para cerca de 64~mm em 2020 (377{,}1~mm e 441{,}6~mm, respectivamente). A temperatura média permaneceu praticamente idêntica entre a sub-região e o estado, com diferenças inferiores a 0{,}3~°C em todos os anos. A radiação solar média, por sua vez, foi consistentemente superior no Extremo Oeste em comparação à média estadual.

\subsubsection*{Área Urbana}

A análise da cobertura de Área Urbana (Figura~\ref{fig:urbano_extremo_oeste_vs_rn}) revelou crescimento expressivo tanto no Extremo Oeste quanto no estado, com trajetórias de expansão praticamente paralelas: de 0{,}6\% em 1990 para 1{,}4\% em 2020 no Extremo Oeste, e de 0{,}6\% para 1{,}5\% no estado, \cite{silveira2016}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{fig/fig2_urbano_extremo_oeste_vs_rn.png}
    \caption[Cobertura de Área Urbana e variáveis climáticas: Extremo Oeste do RN \textit{versus} 
    estado do RN (1990--2020)]{Comparação entre a cobertura de Área Urbana e as variáveis 
    climáticas para o Extremo Oeste do RN e o estado como um todo nos anos de 1990, 2000, 
    2010 e 2020. Valores em laranja referem-se ao Extremo Oeste; valores em vermelho ao 
    estado do RN.}
    \label{fig:urbano_extremo_oeste_vs_rn}
\end{figure}

As variáveis climáticas associadas à expansão urbana mostraram comportamento semelhante ao observado para a Formação Savânica: temperatura média praticamente indiferenciada entre Extremo Oeste e estado (diferenças inferiores a 0{,}2~°C), mas com valores sempre superiores a 26{,}6~°C, precipitação superior na sub-região em todos os períodos e radiação solar com tendência de aumento ao longo do tempo em ambas as escalas. Este último resultado foi aparentemente contraditório frente à precipitação crescente, e sugeriu que os eventos precipitantes na região tendiam a ocorrer predominantemente no período noturno, fora do ciclo diurno.

Tanto para a progressão da Formação Savânica quanto para a Área Urbana foi possível identificar um comportamento em comum, o fato da precipitação e radiação crescerem de forma consistente, mesmo que para o segundo caso seja mais sutil. Esse comportamento, reforça o apresentado na Seção~\ref{section:analise_rn}. Conforme demonstrado na análise do ciclo diurno da precipitação para a faixa central e extremo oeste do estado, os maiores volumes acumulados distribuem-se predominantemente no período noturno e nas primeiras horas da madrugada, com o intervalo entre 06h e 12h respondendo consistentemente pelas menores frações do total precipitado em todas as estações do ano. 

Esse padrão implica que a nebulosidade associada aos eventos precipitantes ocorre fora das horas de maior incidência radiativa, o que permite que precipitação e radiação solar crescentes coexistam sem se anularem.
...
A análise da influência da Formação Florestal (Classe 3) sobre as variáveis climáticas no estado de São Paulo (Figura~\ref{fig:hist_beta_sp_classe3}) revelou padrões distintos para cada variável.
...
A análise da Formação Savânica (Caatinga, Classe 4) no Rio Grande do Norte (Figura~\ref{fig:hist_beta_rn_classe4}) revelou padrões substancialmente distintos daqueles observados em São Paulo, com relações predominantemente fracas e centradas em torno de zero para todas as variáveis climáticas.
...
Para a temperatura, a distribuição foi notavelmente simétrica e centrada em zero ($\beta_{\text{mediana}} = -0{,}0024$ °C/\%), com apenas $0{,}56\%$ dos pontos apresentando significância estatística. Este resultado indicou ausência de relação detectável entre cobertura de Caatinga e temperatura local, contrastando fortemente com o efeito de resfriamento documentado para florestas tropicais úmidas \citep{li2015local, alkama2016biophysical}.
...
A elevada variabilidade climática interanual característica do semiárido nordestino também contribuiu para mascarar possíveis relações entre cobertura vegetal e temperatura. O coeficiente de variação da precipitação anual no semiárido frequentemente excede 40-50\%, com secas plurianuais alternando com períodos de chuvas abundantes em escalas decadais \citep{marengo2017drought}. Esta variabilidade climática natural dominou completamente sobre possíveis sinais associados à mudança de cobertura vegetal, particularmente em séries temporais de três décadas como as utilizadas neste estudo.

A precipitação apresentou ligeira tendência negativa ($\beta_{\text{mediana}} = -6{,}32$ mm/\%), com $9{,}8\%$ dos pontos significativos. Embora este resultado possa sugerir contribuição da vegetação para a precipitação através de reciclagem de umidade, a baixa significância estatística impediu conclusões definitivas. Estudos sobre o papel da vegetação na precipitação em regiões semiáridas são escassos e apresentam resultados contraditórios \citep{sheil2018forest, staal2018forest}, em parte porque os mecanismos de retroalimentação vegetação-precipitação são mais fracos em ambientes com déficit hídrico crônico.

A radiação solar mostrou distribuição praticamente centrada em zero ($\beta_{\text{mediana}} = -0{,}09$ W/m$^2$/\%), embora com $39{,}5\%$ dos pontos significativos. A maior significância relativa para radiação pode refletir o papel da nebulosidade durante a estação chuvosa, que covariou espacialmente com a densidade de vegetação preservada.

\subsection{Influência da Área Urbanizada no Rio Grande do Norte}

A análise da Área Urbanizada (Classe 24) no Rio Grande do Norte (Figura~\ref{fig:hist_beta_rn_classe24}) apresentou padrões intermediários entre a Formação Savânica local e a urbanização paulista, revelando aspectos importantes sobre a modulação do efeito de ilha de calor urbana pelo contexto climático regional.
...
A temperatura apresentou relação positiva fraca ($\beta_{\text{mediana}} = 0{,}072$ °C/\%), porém com apenas $2{,}0\%$ dos pontos significativos. Este resultado contrastou marcadamente com a forte significância observada em São Paulo ($93{,}8\%$). A literatura recente sobre ilhas de calor em climas secos sugere que a intensidade do fenômeno depende criticamente do contraste entre evapotranspiração urbana e rural \citep{zhao2014strong, manoli2019magnitude}. Em regiões onde a vegetação rural já apresenta baixa evapotranspiração devido à limitação hídrica, como no semiárido nordestino, o diferencial térmico urbano-rural é naturalmente atenuado.
...
Resultado particularmente interessante emergiu para a precipitação, que apresentou relação positiva ($\beta_{\text{mediana}} = 194{,}83$ mm/\%, $\beta_{\text{média}} = 889{,}97$ mm/\%), embora com baixa significância estatística ($2{,}9\%$). Este padrão, oposto ao observado em São Paulo, pode estar associado ao fenômeno de intensificação da convecção urbana em ambientes semiáridos.
...
A radiação solar apresentou relação positiva moderada ($\beta_{\text{mediana}} = 3{,}73$ W/m$^2$/\%), com $59{,}9\%$ dos pontos significativos. Este resultado foi consistente com a hipótese de que áreas urbanizadas no semiárido apresentam menor formação de nuvens convectivas devido à supressão da evapotranspiração, resultando em maior transmissividade atmosférica e incidência de radiação solar na superfície.

\subsection{Síntese estatística comparativa}

A Tabela~\ref{tab:beta_cobertura_clima_completa} sintetizou os resultados estatísticos para todas as classes e estados analisados.
...
Segundo, a elevada variabilidade climática interanual do semiárido nordestino, com coeficientes de variação de precipitação frequentemente excedendo 40-50\%, introduziu ``ruído'' substancial nas séries temporais que mascarou possíveis sinais de menor magnitude associados à mudança de cobertura vegetal \citep{marengo2017drought}.

Terceiro, a escala temporal da análise (dados anuais agregados) pode ter sido inadequada para capturar efeitos que se manifestam primariamente em escalas sazonais ou diurnas. Estudos de processo indicam que o efeito de resfriamento da vegetação é máximo durante o período de transpiração ativa (estação chuvosa na Caatinga) e durante as horas de máxima insolação \citep{duveiller2018local}. Análises com resolução temporal mais fina poderiam revelar padrões não detectáveis em médias anuais.

\subsubsection*{Diferenças qualitativas na resposta da precipitação à urbanização}

As diferenças observadas na resposta da precipitação à urbanização entre São Paulo (tendência negativa, $\beta_{\text{mediana}} = -35{,}64$ mm/\%) e Rio Grande do Norte (tendência positiva, $\beta_{\text{mediana}} = +194{,}83$ mm/\%) sugeriram mecanismos físicos distintos operando nos dois contextos climáticos.
...
Em São Paulo, a tendência negativa pode refletir a redução da reciclagem local de umidade devido à diminuição da evapotranspiração em áreas impermeabilizadas. Este mecanismo foi consistente com estudos que demonstram redução de precipitação sobre e a sotavento de áreas urbanas em contextos onde a reciclagem de umidade local é importante para a precipitação total \citep{spracklen2012observations}.

No Rio Grande do Norte, a tendência positiva pode ter resultado da intensificação da convecção urbana em ambiente semiárido. O aquecimento diferencial entre superfícies urbanas e rurais criou gradientes térmicos horizontais que induziram convergência de ar na camada limite urbana, favorecendo o disparo de convecção quando havia umidade atmosférica disponível \citep{shepherd2002atlanta, niyogi2017urbanization}.